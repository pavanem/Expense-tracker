...
@Tag(name = "CSV Export", description = "Export expenses as CSV: single day, range, monthly, yearly, category, or entire history")
public class ExportController {

    private final CsvExportService csvExportService;

    @GetMapping("/export")
    @Operation(summary = "Export expenses as CSV — filtered or all of this user's history ('all=true')")
    public ResponseEntity<byte[]> export(
        ...
        ReportFilterRequest filter = ReportFilterRequest.builder()
                ...
                .month(month).year(year).categoryId(categoryId)
                .paymentMode(paymentMode).merchant(merchant).build();

        CsvExportService.CsvExport export = csvExportService.export(filter, all, userId);
