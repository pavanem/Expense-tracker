...
export default function IncomeCategoryList({ categories, onChanged, onEdit }) {
  ...
  const toggleStatus = async (category) => {
    try {
      if (category.status === 'ACTIVE') {
        await IncomeCategoryService.deactivate(category.id);
        notify(`${category.name} deactivated`);
      } else {
        await IncomeCategoryService.activate(category.id);
        notify(`${category.name} activated`);
      }
      onChanged();
    } catch (err) {
      notifyError(err, 'Could not update income category status');
    }
  };
  ...
  const confirmDelete = async () => {
    ...
    try {
      await IncomeCategoryService.remove(pendingDelete.id);
      notify(`${pendingDelete.name} deleted`);
      onChanged();
    } catch (err) {
      notifyError(err, 'Could not delete income category');
    } finally {
  ...
  if (categories.length === 0) {
    return (
      <Box sx={{ py: 6, textAlign: 'center' }}>
        <Typography color="text.secondary">No income categories yet. Add your first one to get started.</Typography>
      </Box>
    );
  }
  ...
  return (
    <>
      ...
      <ConfirmDialog
        ...
        message={
          pendingDelete
            ? `"${pendingDelete.name}" will be permanently removed. If any income records use this category, deletion will be blocked — deactivate it instead.`
            : ''
        }
        confirmLabel="Delete"
        confirmColor="error"
