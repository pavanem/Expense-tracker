...
export default function IncomeCategoriesScreen() {
  ...
  const handleSave = async () => {
    ...
    try {
      if (editing) {
        await IncomeCategoryService.update(editing.id, { name: name.trim(), status: editing.status });
        showNotification('Category updated', 'success');
      } else {
        await IncomeCategoryService.create({ name: name.trim() });
  ...
  const toggleStatus = async (cat: any) => {
    try {
      ...
      await IncomeCategoryService.update(cat.id, { name: cat.name, status: newStatus });
    ...
    <View style={styles.container}>
      {loading ? (
        ...
        <FlatList
          ...
          ListEmptyComponent={<Text style={styles.empty}>No income categories yet</Text>}
        />
      )}
      ...
      <Portal>
        <Modal visible={modalVisible} onDismiss={() => setModalVisible(false)} contentContainerStyle={styles.modal}>
          <Text variant="titleLarge" style={styles.modalTitle}>
            {editing ? 'Edit Category' : 'Add Income Category'}
          </Text>
